...
let calculateTotal (customer:Customer, spend:Spend) : Total =    
    let discount = 
        match customer with 
        | { IsRegistered = true; IsEligable = true } when spend >= 100m -> spend * 0.1m
        | { IsRegistered = true; IsEligable = isel } when isel = false -> 0m  // pass the variable
        | { IsRegistered = false; IsEligable = false } -> 0m
        | _ -> 0m
    spend - discount
